...
# slug -> source markdown (order = reading order)
POSTS = [
 # --- Nexplane posts series ---
 ...
 # --- Nexplane blog (founder/operational) ---
 ...
 # --- Engineering philosophy (001–050, skipping 019/020/021 which duplicate 008/009/010) ---
...
# ---- index ----
items = "\n".join(
    f'      <a class="post-item" href="/writing/{s}"><span class="pt">{html.escape(t)}</span><span class="pd">{html.escape(d)[:70]}</span></a>'
    for s,t,d in built)
